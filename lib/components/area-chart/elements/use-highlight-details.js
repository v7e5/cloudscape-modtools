...
const useHighlightDetails = ({
  ...
  const detailsTotal = seriesPoints.reduce(
    (total, point) => total + point.value,
    0
  )
  const formattedX = xTickFormatter
    ? xTickFormatter(firstPoint.x)
    : firstPoint.x.toString()
  const seriesTitle = hPoint ? getInternalSeries(hPoint).title : ''
  const formattedY = hPoint
    ? getInternalSeries(hPoint).formatValue(hPoint.value, hPoint.x)
    : ''
  ...
  const getInternalSeries = point => {
    return model.getInternalSeries(model.series[point.index.s])
  }
}

export {
  useHighlightDetails as default
}
