...
import BlockEndLabels, {
  useBLockEndLabels
} from '../internal/components/cartesian-chart/block-end-labels'
...
const ChartContainer = memo(
  ({
    ...
    const [containerWidth, containerWidthRef] =
      useContainerWidth(DEFAULT_CHART_WIDTH)
    ...
    const isPointHighlighted =
      model.interactions.get().highlightedPoint !== null
    ...
    const detailPopoverFooterContent = useMemo(
      () =>
        detailPopoverFooter && highlightedX
          ? detailPopoverFooter(highlightedX[0].x)
          : null,
      [detailPopoverFooter, highlightedX]
    )
    return (
      <CartesianChartContainer
        ...
        chartPlot={
          <ChartPlot
            ...
            height={
              fitHeight
                ? `calc(100% - ${blockEndLabelsProps.height}px)`
                : model.height
            }
            ...
            activeElementKey={
              !highlightDetails?.isPopoverPinned &&
              highlightDetails?.activeLabel
            }
            activeElementRef={
              isPointHighlighted
                ? highlightedPointRef
                : model.refs.verticalMarker
            }
            ...
            <AreaHighlightedPoint
              ...
              model={model}
              ariaLabel={highlightDetails?.activeLabel}
            />
          </ChartPlot>
        }
        popover={
