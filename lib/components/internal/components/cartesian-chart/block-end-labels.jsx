...
const useBLockEndLabels = ({ticks, scale, tickFormatter}) => {
  ...
  for (const {lines} of formattedTicks) {
    height = Math.max(
      height,
      TICK_LENGTH + TICK_MARGIN + lines.length * TICK_LINE_HEIGHT
    )
  }
  return {virtualTextRef, formattedTicks, height}
}

const BlockEndLabels = memo(
  ({
    axis = 'x',
    width,
    height,
