...
const InternalSegmentedControl = memo(
  ({selectedId, options, label, ariaLabelledby, onChange}) => {
    ...
    const moveHighlight = (event, activeIndex) => {
      ...
      handleKey(event, {
        onInlineStart: () =>
          (nextIndex =
            activeIndex === 0 ? enabledSegments.length - 1 : activeIndex - 1),
        onInlineEnd: () =>
          (nextIndex =
            activeIndex + 1 === enabledSegments.length ? 0 : activeIndex + 1)
      })
